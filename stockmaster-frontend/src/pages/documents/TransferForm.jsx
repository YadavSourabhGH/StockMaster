...
const TransferForm = () => {
    ...
    const handleFromWarehouseChange = async (e) => {
        ...
        setFormData(prev => ({ ...prev, fromWarehouse: warehouseId, lines: [{ productId: '', qty: 1 }] }));
        ...
        try {
            ...
            if (res.success && res.data?._id) {
                try {
                    ...
                    toast.success('Transfer created and stock updated successfully');
                } catch (valError) {
                    console.error("Validation error", valError);
                    toast.warning('Transfer created but failed to validate automatically. Please validate from the list.');
                }
            } else {
            ...
            <div className="rounded-lg bg-white p-6 shadow-sm border border-slate-100">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            <label className="mb-1 block text-sm font-medium text-slate-700">To Warehouse</label>
                            <select
                                name="toWarehouse"
